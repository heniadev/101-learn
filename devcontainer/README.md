...
kept in a named Docker volume (`101-learn-devcontainer-home`) so you
...
user-defined network (`101-learn-devcontainer-net`) the devcontainer
...
- **To reset it**, `docker compose -f devcontainer/docker-compose.yml down -v`
  drops the named volume (`101-learn-devcontainer-pgdata`) — the next
...
default `101-learn_dev` — nothing extra to configure, `run.sh` creates
...
subsystem (`git credential approve`) before starting Claude Code. If the file
is absent, git push/pull inside the container simply isn't authenticated —
everything else still works.

### Commit identity

The `author:` block is a **separate identity from the push account above**,
and the distinction is not cosmetic. `Signed-off-by` is a Developer
Certificate of Origin statement (<https://developercertificate.org/>); it is
also what carries the contribution representation Apache-2.0 §5 assumes. Only
a person can make either. A bot account signing off on its own commits
produces the trailer without the thing the trailer is for.

So the two identities are wired to different places:

| | Comes from | Used for |
|---|---|---|
| `git.username` / `git.password` | the dedicated bot account | authenticating push/pull and `tea`; a `Co-authored-by:` trailer crediting the agent |
| `author.fullname` / `author.email` | **you**, the human directing the work | `user.name`/`user.email`, so you are the commit `Author:` and the `Signed-off-by:` signer |

Both trailers are attached by `.githooks/prepare-commit-msg`, which the
entrypoint enables globally via `core.hooksPath=.githooks`. A commit written
inside the container therefore ends up as:
...
Co-authored-by: claude <claude@your-git-host>
Signed-off-by: Your Name <you@example.com>
```

with `Author:` being you. Omit the `author:` block and `run.sh` warns, then
falls back to the old behaviour — the bot authors and signs off on its own
work, which is exactly the thing worth avoiding.

On your **host** clone the same hook needs enabling once, since
`core.hooksPath` is not something a repository can set for you:

```bash
git config core.hooksPath .githooks
```

Note that `git config format.signOff true` does **not** do this. It applies
to `git format-patch` and `git send-email`; `git commit` ignores it. The hook
(or typing `git commit -s` every single time) is what actually reaches a
commit.
...
- **DNS still has to work.** The container runs on a dedicated user-defined
  Docker network (`101-learn-devcontainer-net`) instead of the default
...
| Session/config persistence across runs | Named Docker volume (`101-learn-devcontainer-home`) mounted at the container's `$HOME`, survives across `--rm` runs | ✅ Met |
